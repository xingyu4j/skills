...
name: nuxt-seo
description: |
  全部 8 个 Nuxt SEO 模块及 Pro 模块的综合指南：@nuxtjs/seo、nuxt-robots、nuxt-sitemap、nuxt-og-image、nuxt-schema-org、nuxt-link-checker、nuxt-seo-utils、nuxt-site-config 以及 Pro 模块（AI Ready、Skew Protection、SEO Pro MCP）。适用于构建 SEO 优化的 Nuxt 应用、实现 robots.txt 和站点地图、生成 Open Graph 图片、添加 Schema.org 结构化数据、管理 meta 标签、检查链接、配置全站 SEO 设置，或实现渲染模式、规范 URL、Twitter Cards 和 IndexNow 等高级 SEO 功能。
